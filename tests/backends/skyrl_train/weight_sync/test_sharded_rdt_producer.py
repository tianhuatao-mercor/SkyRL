...
still reading it. Freeing is a PER-GROUP BARRIER: every consumer signals
`free_group(gi)` at every owner of the group exactly once per sync, and the
group is released when the count reaches the consumer total handed to
`begin_sync` — one uniform integer, no routed per-producer targets.
...
covered by the GPU tests in `the fork's test_weight_transfer.py`.
"""
...
class TestFreeBarrier:
    ...
    def test_group_is_held_until_the_last_consumer_signals(self, server_factory):
        """Every consumer signals every owner; releasing on the first signal
        would drop storage other consumers are still reading."""
        server = server_factory()
        server.begin_sync(3)
        _publish(server, GI_A, GROUP_A)
    ...
    def test_lookahead_one_holds_at_most_two_groups_resident(self, server_factory, gather_engine):
        ...
        def _consumer():
            for gi in range(n_groups):
                target = min(gi + 1, n_groups - 1)  # N+1 must be pullable before N frees
                ...
                while time.monotonic() < deadline:
                    ...
                    failures.append(f"group {target} never published while {gi} was still unfreed")
                    return
...
class TestEndSync:
    ...
    def test_a_gather_error_releases_a_blocked_end_sync(self, server_factory):
        ...
        threading.Thread(target=lambda: (server.end_sync(), done.set()), daemon=True).start()
...
class TestStallWatchdog:
    ...
    def test_a_blocked_wait_freed_fails_instead_of_hanging(self, server_factory):
        ...
        server.begin_sync(1)
        _publish(server, GI_A, GROUP_A)  # the engine's credit gate now waits; nobody will signal
    ...
    def test_end_sync_fails_when_a_consumer_never_signals(self, server_factory):
        """The exact stuck-consumer signature: the group is published and one of
        the two consumers that owed a signal never sends it."""
...
class TestConcurrentPublishAndFree:
    ...
    def test_concurrent_signals_of_one_group_release_it_exactly_once(self, server_factory):
        """All consumers signaling at once: the release must happen on the
...
class TestExportRing:
    ...
    def _roundtrip(self, held, slot=0):
        ...
        # Rebuild the way publish_group does, from the slot alone.
        return {
    ...
    def test_a_reused_slot_reallocates_and_still_rebuilds(self):
        """Slot reuse across groups is the point; a later, larger group must
        grow the slot without corrupting its own layout."""
        eng = self._engine()
        first = [("a", torch.randn(4, 4).bfloat16())]
        eng._pack_group_for_export(first, 0)
        ...
        name, (_sid, dt, shape, stride, off) = next(iter(views.items()))
...
class TestExportRingSlotSafety:
    ...
    def test_distinct_inflight_groups_never_share_a_slot(self, server_factory, gather_engine):
...
class TestSharedServeSlots:
    ...
    def test_a_sharer_that_never_arrives_trips_the_watchdog(self, sharing_server):
        """A consumer that never reaches the rendezvous has no other detector;
        it waits on the same channel as the free barrier, so it fails with a
