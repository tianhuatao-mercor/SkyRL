...
class ShardedRDTWeightTransferEngine(
    ...
    def _resolve_producers(self, init_info: ShardedRDTWeightTransferInitInfo) -> None:
        ...
        if name_owner_class and len(name_owner_class) != len(init_info.names):
            raise RuntimeError(
                f"Sharded RDT engine: {len(name_owner_class)} owner-class entries " f"for {len(init_info.names)} names."
            )
        router = RdtRouter(
            len(producer_names),
            self._num_consumers(),
        ...
        # position in these lists, so a second init_transfer_engine must not
        # shift them.
        actors: list[Any] = []
        methods: list[Any] = []
