...
class _RDTProducerServer:
    ...
    def __init__(
        ...
        *,
        ...
        # Every consumer signals free_group(gi) at every owner, once per sync;
        # the group frees when the count reaches begin_sync's consumer total —
        # one uniform target, no routed per-producer ones. Signals may precede
        # the publish, which completes them.
        ...
        self._consumer_ids: set[int] | None = None
        self._sharing_active = False
        self._sharers: dict[int, frozenset] = {}  # group -> members
    ...
    def begin_sync(self, consumer_count: int, consumer_ids: list | None = None) -> None:
        """Reset per-sync free/backpressure state and set this sync's barrier
        target.

        ``consumer_count`` is how many consumers take part in the sync. Required, no
        ...
        ``consumer_ids`` is that same set enumerated, which is what
        ...
        with self._cache_cond:
            ...
            self._consumer_count = max(1, int(consumer_count))
            self._consumer_ids = set(int(c) for c in consumer_ids) if consumer_ids is not None else None
            ...
            self._sharing_active = (
                self._share_width > 0 and self._consumer_ids is not None and len(self._consumer_ids) > self._share_width
            )
            self._sharers.clear()
            self._gens.clear()
    ...
    def publish_group(self, group_idx: int, entries: tuple) -> None:
        ...
        with self._cache_cond:
            ...
            if self._free_counts.get(group_idx, 0) >= self._consumer_count:
    ...
    def wait_freed(self) -> list[int]:
        ...
        with self._cache_cond:
            ...
            if not self._freed_pending:
                raise RuntimeError("gather errored while waiting for a freed-group credit: " f"{self._gather_error!r}")
    ...
    def free_group(self, group_idx: int) -> None:
        ...
        with self._cache_cond:
            ...
            if gi in self._group_names and count >= self._consumer_count:
    ...
    def _sharers_of(self, sg: int) -> frozenset:
        """The consumers of group ``sg``: the rendezvous width. Derived from
        ``begin_sync``'s consumer set. Caller holds ``_cache_cond``."""
        cached = self._sharers.get(sg)
        if cached is None:
            if not self._sharing_active or self._consumer_ids is None:
                cached = frozenset((sg,))
            else:
                cached = frozenset(c for c in self._consumer_ids if c % self._share_width == sg) or frozenset((sg,))
    ...
    def _await_shared_pack(self, gen: _SharedPack) -> torch.Tensor:
        ...
        with self._cache_cond:
            ...
            if not gen.done:
                raise RuntimeError(f"gather errored while awaiting a shared pack: " f"{self._gather_error!r}")
            return gen.blob
    ...
    def rdt_produce_weights_batched(self, specs: list, consumer_id: int = 0, seq: int = -1):
        ...
        if self._served_names is not None:
            ...
            if unserved:
                ...
                raise RuntimeError(
                    f"pull routed to the wrong producer: {unserved[:3]} " f"({len(unserved)} names) are not served here"
                )
        with self._cache_cond:
            self._wait_for(
...
class ShardedRDTTrainerWeightTransferEngine(TrainerWeightTransferEngine[ShardedRDTTrainerInitInfo]):
    ...
    def _resolve_ownership(self, world: int, rank: int) -> None:
        ...
        # rank-identical inputs, so every rank builds the identical table.
    ...
    def send_weights(self) -> None:
        ...
        The consumer set is the provisioned fleet, frozen at ``trainer_init``:
        every rank derives it identically from ``num_consumers``, which matters
        because the ranks share the gather collectives.
        """
        assert self.source is not None
        consumer_ids = list(range(self._init_info.num_consumers))
        if not self.is_sender:
            self._run_gather_loop(update_future=None, consumer_ids=consumer_ids)
            return
        ...
        with ThreadPoolExecutor(max_workers=1) as exe:
            ...
            self._run_gather_loop(update_future=future, consumer_ids=consumer_ids)
            future.result()  # surface inference-side errors

        self.client.finish_weight_update()

    def _run_gather_loop(self, update_future, consumer_ids: list[int]) -> None:
        ...
        credit releases when every consumer has signaled the group). So
        ...
        assert self.source is not None  # guaranteed by trainer_init
        # The IDS ride along with the count: the free barrier needs only the
        # count, but the slot-sharing rendezvous needs to know WHICH consumers
        # take part.
        self._rpc("begin_sync", len(consumer_ids), consumer_ids)
