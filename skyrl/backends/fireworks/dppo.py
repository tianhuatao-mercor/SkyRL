...
def make_binary_tv_dppo_loss(
    ...
    *,
    ...
    if not math.isfinite(delta_low) or not math.isfinite(delta_high) or delta_low < 0 or delta_high < 0:
    ...
    def loss_fn(
        ...
        if len(data) != len(specs) or len(logprobs_list) != len(specs):
            raise ValueError("Fireworks DPPO datum/logprob counts must match the prepared specs")

        total_loss: torch.Tensor | None = None
        clipped_tokens = 0
        trainable_tokens = 0
        for row_index, (spec, current_logprobs) in enumerate(zip(specs, logprobs_list, strict=True)):
            ...
            if not torch.isfinite(current_logprobs).all():
                raise ValueError(f"Fireworks DPPO current logprobs contain non-finite values in row {row_index}")
            ...
            ratio = torch.exp(torch.clamp(compute_logprobs - behavior_logprobs, -20.0, 20.0))
            with torch.no_grad():
                probability_delta = torch.exp(compute_logprobs) - torch.exp(behavior_logprobs)
...
def build_tinker_binary_tv_dppo_request(
    ...
    *,
    ...
    try:
        ...
        raise ImportError(
            "Fireworks DPPO construction requires the 'tinker' package; " "install SkyRL with the Fireworks extra"
        ) from exc
    ...
    datums = [
        tinker.Datum(
            ...
            loss_fn_inputs={"target_tokens": tinker.TensorData(data=list(spec.target_tokens), dtype="int64")},
        )
        for spec in specs
    ]
